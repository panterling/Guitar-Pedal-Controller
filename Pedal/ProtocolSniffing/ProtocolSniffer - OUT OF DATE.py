...
endpointWriteRef.write(bytearray.fromhex(hexPayload))
time.sleep(5)
...
while True:
    ...
    lowestValueStateHex = getStateHex(rxQueue, endpointWriteRef)

    ok = input("Move to highest Value")
    highestValueHex = str(getLastRxMsg())[2:-1]
    highestValueStateHex = getStateHex(rxQueue, endpointWriteRef)
    ...
    if min(changingIdxs) in BASIC_CASE_IDX_SET[0:2] and max(changingIdxs) == BASIC_CASE_IDX_SET[-1]:
        ...
        endpointWriteRef.write(payload)

        # Pause then Request State
        time.sleep(1)
        state = getStateHex(rxQueue, endpointWriteRef)
